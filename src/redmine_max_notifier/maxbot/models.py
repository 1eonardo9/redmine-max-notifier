...
class MessageBody(MaxModel):
    """Тело входящего/отправленного сообщения (то что MAX возвращает нам).

    Соответствует части ответа POST /messages -> message.body.
    Полный набор полей у MAX больше; заводим только то, что реально
    используем сейчас (mid и seq пригодятся для отслеживания).
    """
...
class SentMessage(MaxModel):
    ...
    Полный объект message включает много полей (recipient, sender, timestamp,
    stat и т.д.). Пока берём только те, что реально понадобятся: body с
    идентификатором отправленного сообщения — этого достаточно, чтобы
    подтвердить факт отправки и опционально залогировать mid.
    """
