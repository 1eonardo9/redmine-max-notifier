...
class StatusChangedEvent(EventBase):
    ...
    Имена статусов резолвит рендерер шаблона по заранее подгруженному
    словарю /issue_statuses.json — сюда их не тащим, чтобы модель
    события не зависела от того, знает ли детектор имена.
