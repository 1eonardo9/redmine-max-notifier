...
- **раз в минуту** — свежие изменения: новые задачи и правки
  существующих;
...
| Обновление задачи | в журнале изменились статус / приоритет / срок, появился комментарий или вложение |
| Приближение срока | до `due_date` осталось ≤ N дней (по умолчанию 3) |

Одна journal-запись Redmine — одно уведомление: «сменил статус и
приоритет с комментарием одной формой» приедет одним сообщением,
а не тремя. Закрытие задачи — обычная смена статуса. Приватные
заметки не шлём (вложения приватной заметки тоже скрыты).

В новой задаче и напоминании о сроке исполнитель и соисполнители
получают @упоминание (если сопоставлены с MAX-пользователями,
см. [шпаргалку](cheatsheets/mentions.md)). Обновления задач
не пингуют никого — чтобы не частить.
...
- **`REDMINE_BASE_URL_PUBLIC`** — внешний адрес Redmine для ссылок
  в сообщениях. Сейчас шаблоны ссылок не печатают (решили «пока не
  нужно»), переменная оставлена на случай возврата фичи.
...
**Redeploy** (код уже стоит): остановить сервис, вычистить
`/opt/redmine-max-notifier` **кроме `.venv`**, и только потом распаковать
тарбол. tar кладёт файлы поверх и не удаляет те, что из репозитория
исчезли — переименованный модуль оставил бы на проде обе версии.
После распаковки — `uv sync --frozen` (шаг 2) и старт сервиса.
Миграции — только если менялась схема БД.
...
# uv ставим официальным установщиком (в репах Debian 13 пакета нет)
# в системный PATH: под sudo пользовательский ~/.local/bin не подхватывается
...
#
# UV_PYTHON_INSTALL_DIR обязателен: без него uv кладёт managed-Python
# в /root/.local/share/uv/python, .venv/bin/python — симлинк туда,
# и сервис падает на старте с Permission denied (700 на /root +
# ProtectHome=true в юните).
cd /opt/redmine-max-notifier
sudo env UV_PYTHON_INSTALL_DIR=/opt/uv/python uv sync --frozen
```
...
[rmn](cheatsheets/rmn.md) (прод-обёртка CLI: mapping, routing, логи, рестарт).
...
- [x] Этап 8. Финальные тесты и деплой (systemd)
- [x] Этап 9. Расширяемость: приоритет и срок в уведомлениях,
      одно сообщение на journal-запись, @пинг соисполнителей
- [ ] Дальше: подписки пользователей на конкретные задачи
